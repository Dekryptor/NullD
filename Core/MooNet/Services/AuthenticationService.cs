...
using NullD.Core.LogNet.Authentication;
using NullD.Core.Cryptography;
...
namespace NullD.Core.LogNet.Services
{
    ...
    public class AuthenticationService : bnet.protocol.authentication.AuthenticationServer, IServerService
    {
        ...
        public LogNetClient Client { get; set; }
        ...
        public override void Logon(Google.ProtocolBuffers.IRpcController controller, bnet.protocol.authentication.LogonRequest request, Action<bnet.protocol.NoData> done)
        {
            ...
            {
                Logger.Error("Client [{0}] doesn't match required version {1}, disconnecting..", request.Email, VersionInfo.LogNet.RequiredClientVersion);
                ...
                // when I compare NullD's output. That could be the reason. /raist.
        ...
        public override void ModuleMessage(Google.ProtocolBuffers.IRpcController controller, bnet.protocol.authentication.ModuleMessageRequest request, Action<bnet.protocol.NoData> done)
        {
            ...
            switch (module)
            {
                case LogNetClient.StreamedModule.Password:
                    if (command == 2)
                    ...
                    break;
                case LogNetClient.StreamedModule.Token:
                    ...
                    moduleHandle.SetRegion(VersionInfo.LogNet.Regions[VersionInfo.LogNet.Region])
                        .SetUsage(0x61757468); // auth - RiskFingerprint.dll or Agreement.dll

                    if (this.Client.HasAgreements())
                    {
                        moduleHandle.SetHash(ByteString.CopyFrom(VersionInfo.LogNet.AgreementHashMap[this.Client.Platform]));
                        this.Client.LastRequestedModule = LogNetClient.StreamedModule.Agreement;
                    }
                    else
                    {
                        moduleHandle.SetHash(ByteString.CopyFrom(VersionInfo.LogNet.RiskFingerprintHashMap[this.Client.Platform]));
                        moduleLoadRequest.SetMessage(ByteString.Empty);
                        this.Client.LastRequestedModule = LogNetClient.StreamedModule.RiskFingerprint;
                    }
                    ...
                    break;
                case LogNetClient.StreamedModule.RiskFingerprint:
                    Logger.Trace("Completing Authentication.");
                    this.Client.AuthenticationComplete();
                    break;
                case LogNetClient.StreamedModule.Agreement:
                    switch (this.Client.LastAgreementSent)
                    {
                        case LogNetClient.AvailableAgreements.EULA:
                            this.Client.Agreements.Add(LogNetClient.AvailableAgreements.EULA, true);
                            break;
                        case LogNetClient.AvailableAgreements.TOS:
                            this.Client.Agreements.Add(LogNetClient.AvailableAgreements.TOS, true);
                            break;
                        case LogNetClient.AvailableAgreements.RMAH:
                            this.Client.Agreements.Add(LogNetClient.AvailableAgreements.RMAH, true);
                            break;
                        default:
        ...
        public override void ModuleNotify(Google.ProtocolBuffers.IRpcController controller, bnet.protocol.authentication.ModuleNotification request, Action<bnet.protocol.NoData> done)
        {
            ...
            {
                case LogNetClient.StreamedModule.Thumbprint:
                    moduleLoadRequest.SetModuleHandle(bnet.protocol.ContentHandle.CreateBuilder()
                        ...
                        .SetHash(ByteString.CopyFrom(VersionInfo.LogNet.PasswordHashMap[this.Client.Platform])))
                        .SetMessage(ByteString.CopyFrom(AuthManager.OngoingAuthentications[this.Client].LogonChallenge));

                    this.Client.LastRequestedModule = LogNetClient.StreamedModule.Password;
                    this.Client.MakeRPC(() => bnet.protocol.authentication.AuthenticationClient.CreateStub(this.Client).ModuleLoad(null, moduleLoadRequest.Build(), ModuleLoadResponse));
                    break;
                case LogNetClient.StreamedModule.Agreement:
                    if (this.Client.HasAgreements())
                    ...
                    else
                    {
                        moduleLoadRequest.SetModuleHandle(bnet.protocol.ContentHandle.CreateBuilder()
                            ...
                            .SetHash(ByteString.CopyFrom(VersionInfo.LogNet.RiskFingerprintHashMap[this.Client.Platform])))
