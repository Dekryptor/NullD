...
namespace NullD.Core.GS.Common
{
    ...
    public class InventoryGrid : IRevealable
    {
        ...
        private InventorySize GetItemInventorySize(Item item)
        {
            ...
            {
                if (item.ItemType.Name.ToLower().Contains("belt"))
                    return new InventorySize() { Width = 1, Height = 1 };
        ...
        public Types.Math.Vector2D GetItemSize(Item item)
        {
            if (Item.IsWeapon(item.ItemType) || Item.IsArmor(item.ItemType) || Item.IsOffhand(item.ItemType))
            {
                if (item.ItemType.Name.ToLower().Contains("belt"))
                    return new Types.Math.Vector2D { X = 1, Y = 1 };
                return new Types.Math.Vector2D { X = 1, Y = 2 };
            }
            return new Types.Math.Vector2D { X = 1, Y = 1 };
        }
        ...
        public bool AddSelledItem(Item item)
        {
            return AddSellItem(-1, -1, item);
        }

        public Item GetItemByLoc(int row, int col)
        {
            Item item = null;

            foreach (var itm in Items.Values)
                if (itm.InventoryLocation.X == col)
                    if (itm.InventoryLocation.Y == row)
                        item = itm;
        ...
        public bool AddSellItem(int minRow, int maxRow, Item item)
        {
            InventorySlot? slot = FindSlotForItem(minRow, maxRow, item);
            if (slot.HasValue)
            {
                AddSelledItem(item, slot.Value.Row, slot.Value.Column);
