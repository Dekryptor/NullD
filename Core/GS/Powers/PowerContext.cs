...
using NullD.Net.GS.Message.Definitions.Actor;
...
namespace NullD.Core.GS.Powers
{
    public class PowerContext
    {
        ...
        private Func<Actor, bool> _EnemyActorFilter
        {
            get
            {
                if (User is Player || User is Minion || User is Enchantress || User is Scoundrel || User is Templar)
                    return (actor) => actor is Monster || actor is Desctructible || actor is DesctructibleLootContainer ||  actor is Barricade;
               else if (User is Gizmo)
                    return (actor) => actor is Player || actor is Minion || User is Enchantress || User is Scoundrel || User is Templar || actor is Monster;
                else
                    return (actor) => actor is Player || actor is Minion || actor is Enchantress || User is Scoundrel || User is Templar ||  User is CaptainRumford;
            }
        }
        ...
        private Func<Actor, bool> _AllyActorFilter
        {
            get
            {
                if (User is Player || User is Minion || User is Enchantress || User is Scoundrel || User is Templar || User is CaptainRumford)
                    return (actor) => actor is Player || actor is Minion || actor is Enchantress || actor is Scoundrel || actor is Templar;
                else if (User is Gizmo)
                    return (actor) => actor is Player || actor is Minion || User is Enchantress || User is Scoundrel || User is Templar || actor is Monster;
                else
