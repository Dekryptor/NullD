...
using NullD.Core.GS.Actors.Implementations.Minions;
...
namespace NullD.Core.GS.Powers.Implementations
{
    //TODO: all runes
    ...
    public class WizardMeteor : PowerScript
    {
        public override IEnumerable<TickTimer> Run()
        {
            ...
            // HACK: NullD's 100ms update rate is a little to slow for the impact to appear right on time so
    ...
    public class WizardTeleport : PowerScript
    {
        public override IEnumerable<TickTimer> Run()
        {
            if (!User.World.CheckLocationForFlag(TargetPosition, NullD.Common.MPQ.FileFormats.Scene.NavCellFlags.AllowWalk))
            {
    ...
    public class DiamondSkin : Skill
    {
        ...
        class DiamondSkinBuff : PowerBuff
        {
            ...
            public override bool Apply()
            {
                if (!base.Apply())
                    return false;
                User.Attributes[NullD.Net.GS.Message.GameAttribute.Look_Override] = 0x061F7489;
            ...
            {
                base.Remove();
                User.Attributes[NullD.Net.GS.Message.GameAttribute.Look_Override] = 0;
