...
using NullD.Net.GS.Message.Definitions.NPC;
using NullD.Net.GS;
...
namespace NullD.Core.GS.Actors
{
    public class InteractiveNPC : NPC, IMessageConsumer
    {
        ...
        protected override void quest_OnQuestProgress(Quest quest) // shadows Actors'NullD.Core.GS.Actors.InteractiveNPC.quest_OnQuestProgress(NullD.Core.GS.Games.Quest)'
        {
            ...
            // brutalConversationAddOnQuestEnd(quest);-0.
        }

        // first hack erekose
        public void AddMustConversation()
        {
            bool questConversation = false;
            var LeahToConversatio = this.World.GetActorBySNO(4580);

            if (this.ActorSNO.Id == 4580 && this.DynamicID == 83)
            {
                foreach (var player in this.World.Game.Players.Values)
                {
                    ...
                    if (dbQuestProgress.ActiveQuest == 72095)
                    {
                        if (dbQuestProgress.StepOfQuest == -1)
                        {
                            ...
                            this.Conversations.Add(new ConversationInteraction(198541));
        ...
        private void brutalConversationAddOnQuestEnd(Quest quest)
        {
            if (ActorSNO.Id == 4580) // for now only leah is concerned :p
            {
                ...
                if (quest.IsDone())
                {
                    ...
                    var conv_assets = NullD.Common.MPQ.MPQStorage.Data.Assets[Common.Types.SNO.SNOGroup.Conversation];
                    var convs = from conv_asset in conv_assets.Values
                                where (conv_asset.Data as NullD.Common.MPQ.FileFormats.Conversation).SNOQuest == quest.SNOHandle.Id
                                select (conv_asset.Data as NullD.Common.MPQ.FileFormats.Conversation);
                ...
                if (quest.IsDone())
                {
                    if (ConversationList != null) // this is from Actor
                    {
                        ...
                        foreach (var conversation in Conversations) // this is in the InteractiveNPC
                            if (NullD.Common.MPQ.MPQStorage.Data.Assets[Common.Types.SNO.SNOGroup.Conversation].ContainsKey(conversation.ConversationSNO))
                                if ((NullD.Common.MPQ.MPQStorage.Data.Assets[Common.Types.SNO.SNOGroup.Conversation][conversation.ConversationSNO].Data as NullD.Common.MPQ.FileFormats.Conversation).I0 == 1)
