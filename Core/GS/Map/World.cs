...
namespace NullD.Core.GS.Map
{
    public sealed class World : DynamicObject, IRevealable, IUpdateable
    {
        ...
        // Environment
        public NullD.Common.MPQ.FileFormats.Environment Environment { get; private set; }
            ...
            : base(game.NewWorldID)
        {
            ...
            Environment = ((NullD.Common.MPQ.FileFormats.World)NullD.Common.MPQ.MPQStorage.Data.Assets[SNOGroup.Worlds][snoId].Data).Environment;
        ...
        public void SpawnRandomItemDrop(Actor source, Player player)
        {
            ...
            //Шанс дропа
            if (RandomHelper.NextDouble() < NullD.Net.GS.Config.Instance.ItemDropRate / 6)
                DropItem(source, null, item);
        }

        public void SpawnLegendaryItemDrop(Actor source, Player player)
        {
            var item = ItemGenerator.GenerateLegendaryRandom(player);
            if ((item is SpellRune) /*&& (item.Attributes[GameAttribute.Rune_Rank] == 0)*/)
            {
                // favor player's class in attuned runes // TODO: remove or move this
                if (RandomHelper.NextDouble() > 0.6f)
        ...
        public void SpawnGold(Actor source, Player player)
        {
            ...
            var item = ItemGenerator.CreateGold(player, RandomHelper.Next(1, 20) * (int)NullD.Net.GS.Config.Instance.GoldRate);// somehow the actual ammount is not shown on ground /raist.

            if (RandomHelper.NextDouble() < NullD.Net.GS.Config.Instance.GoldDropRate / 100)
        ...
        public void ReplaceDoorToAutoDoor(Actor Door)
        {
            var AutoDoor = new DoorProximity(this, Door.ActorSNO.Id, new TagMap());
            //monster.Scale = 1.35f; // this shoudln't be here
            AutoDoor.RotationAxis = Door.RotationAxis;
            AutoDoor.RotationW = Door.RotationW;
            AutoDoor.EnterWorld(Door.Position);
            Door.Destroy();
        }
        public Actor SpawnMonsterWithGet(int monsterSNOId, Vector3D position)
        {
            var monster = ActorFactory.Create(this, monsterSNOId, new TagMap());
            //monster.Scale = 1.35f; // this shoudln't be here
            monster.EnterWorld(position);
        ...
        {
            var groupHash = NullD.Common.Helpers.Hash.StringHashHelper.HashItemName(group);
            foreach (var actor in this._actors.Values)
            {
                if (actor.Tags != null)
        ...
        public bool CheckLocationForFlag(Vector3D location, NullD.Common.MPQ.FileFormats.Scene.NavCellFlags flags)
        {
