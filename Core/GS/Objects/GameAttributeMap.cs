...
using NullD.Net.GS.Message.Definitions.Attribute;
using System.Linq;
using NullD.Net.GS.Message;
using NullD.Net.GS;
using NullD.Common.Logging;

namespace NullD.Core.GS.Objects
{
        ...
        */
        ...
        private List<GameMessage> GetMessageListFromEnumerator(IEnumerator<KeyId> e, int count, bool level)
        {
            ...
            if (count == 1)
            {
                ...
                msg.Field1 = new NullD.Net.GS.Message.Fields.NetAttributeKeyValue();
            ...
            else
            {
                ...
                if (count >= 15)
                {
                    for (; count >= 15; count -= 15)
                    {
                        ...
                        msg.ActorID = _parent.DynamicID;
                        msg.atKeyVals = new NullD.Net.GS.Message.Fields.NetAttributeKeyValue[15];
                        for (int i = 0; i < 15; i++)
                            msg.atKeyVals[i] = new NullD.Net.GS.Message.Fields.NetAttributeKeyValue();
                        for (int i = 0; i < 15; i++)
                        {
                            KeyId keyid;
                            if (!e.MoveNext())
                ...
                if (count > 0)
                {
                    AttributesSetValuesMessage msg = new AttributesSetValuesMessage();
                    msg.ActorID = _parent.DynamicID;
                    msg.atKeyVals = new NullD.Net.GS.Message.Fields.NetAttributeKeyValue[count];
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        var kv = new NullD.Net.GS.Message.Fields.NetAttributeKeyValue();
                        msg.atKeyVals[i] = kv;
