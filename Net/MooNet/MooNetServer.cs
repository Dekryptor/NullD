...
{
    public sealed class LogNetServer : Server
    {
        ...
        public LogNetServer()
        {
            this.OnConnect += LogNetServer_OnConnect;
            this.OnDisconnect += LogNetServer_OnDisconnect;
            this.DataReceived += (sender, e) => LogNetRouter.Route(e);
            this.DataSent += (sender, e) => { };
        }

        private void LogNetServer_OnConnect(object sender, ConnectionEventArgs e)
        {
            Logger.Trace("LogNet-Client connected: {0}", e.Connection.ToString());
            e.Connection.Client = new LogNetClient(e.Connection);
        }

        private void LogNetServer_OnDisconnect(object sender, ConnectionEventArgs e)
        {
            var client = ((LogNetClient)e.Connection.Client);
            ...
            PlayerManager.PlayerDisconnected((LogNetClient)e.Connection.Client);
        }
        ...
        public override void Run()
        {
            ...
            Logger.Info("LogNet-Server is listening on {0}:{1}...", bindIP, Config.Instance.Port);
            Console.Beep();
        }
    }
}
