...
using NullD.Core.Cryptography.SSL;
using NullD.Core.LogNet.Accounts;
using NullD.Core.LogNet.Authentication;
using NullD.Core.LogNet.Channels;
using NullD.Core.LogNet.Objects;
using NullD.Core.LogNet.Services; // Service
using NullD.Net.GS;
using NullD.Net.LogNet.Packets;
using NullD.Net.LogNet.RPC;
using OpenSSL;

namespace NullD.Net.LogNet
{
    public class LogNetClient : IClient, IRpcChannel
    {
        ...
        /// LogNet Layer Stream
        /// </summary>
        public LogNetBuffer IncomingLogNetStream { get; private set; }
        ...
        public LogNetClient(IConnection connection)
        {
            this.Platform = ClientPlatform.Unknown;
            this.Locale = ClientLocale.Unknown;
            this.MOTDSent = false;

            this.IncomingLogNetStream = new LogNetBuffer();
        ...
        public void CallMethod(MethodDescriptor method, IRpcController controller, IMessage request, IMessage responsePrototype, Action<IMessage> done)
        {
            ...
            var packet = new PacketOut((byte)serviceId, LogNetRouter.GetMethodId(method), (uint)token, this._listenerId, request);
        ...
        void OnTLSAuthentication(IAsyncResult result)
        {
            ...
            // Note that this is only an issue when the game client is running on the same machine as NullD.
            Thread.Sleep(100);
            ...
            NullD.Core.LogNet.Online.PlayerManager.PlayerConnected(this);

            NullD.Core.LogNet.Authentication.AuthManager.SendAccountSettings(this);
